...
{
    uint16 time_count = 0;
    while(gpio_get_level(WIRELESS_UART_RTS_PIN))                                      // 如果RTS为低电平，则继续发送数据
    {
        if(time_count++ > WIRELESS_UART_TIMEOUT_COUNT)
            return 1;                                                           // 模块忙,如果允许当前程序使用while等待 则可以使用后面注释的while等待语句替换本if语句
        system_delay_ms(1);
    }
    uart_write_byte(WIRELESS_UART_INDEX, data);                                 // 发送最后的数据
...
uint32 wireless_uart_send_buff ( uint8 *buff, uint32 len)
{
    ...
    {
        time_count = 0;
        while(gpio_get_level(WIRELESS_UART_RTS_PIN) && time_count++ < WIRELESS_UART_TIMEOUT_COUNT)                  // 如果RTS为低电平，则继续发送数据
            system_delay_ms(1);
        if(time_count >= WIRELESS_UART_TIMEOUT_COUNT)
    ...
    if(time_count >= WIRELESS_UART_TIMEOUT_COUNT)
        return len;                                                                             // 模块忙,如果允许当前程序使用while等待 则可以使用后面注释的while等待语句替换本if语句
    uart_write_buffer(WIRELESS_UART_INDEX, buff, len);                                          // 发送最后的数据
...
uint32 wireless_uart_send_string ( uint8 *str)
{
    ...
    uart_write_buffer(WIRELESS_UART_INDEX, str, len);                           // 发送最后的数据
